...
void main() {
  ...
  if (l <= r * r) {
    for (int side = 0; side <= 1; side++) {
      ...
      float lighting = pow(dotNL,diffuse)*mapBrightness;
    ...
    float outerGlowRadius = 0.3;
    float outD = sqrt(outerGlowRadius/(l - r * r));
    glowFactor = smoothstep(0.5, 0.97, outD / (1. + outD));
  }

  gl_FragColor = color + vec4(glowFactor * glowColor * 1.32, glowFactor);
}
